    ...
    class IlDagilimGumrukDahil
    {
            ...
            do
            {
                try
                {
                    ...
                    if (!Onbellek.YasakKalktiMi && !BosXmllerOkundu)
                    {
                        ...
                        string yeniDosyaIcerik = BosXmlOku(appRoot + "AA_Secim2015_SecimCevresiGumrukDahil.txt");
                    ...
                    if (File.Exists(Onbellek.gPath + "AA_Secim2015_SecimCevresiGumrukDahil.txt"))
                    {
                        string yeniDosyaIcerik = OnChangedIlDagilimiXml(Onbellek.gPath + "AA_Secim2015_SecimCevresiGumrukDahil.txt");
                        if (!string.IsNullOrEmpty(yeniDosyaIcerik))
                        {
                            try
                            {
                ...
                {
                    bilgilendirme.EventlogaYaz("Il Dagilim Gumrukdahil BB Hata   " + ex.ToString(), EventLogEntryType.Error, 1);
                }
        ...
        public static string OnChangedIlDagilimiXml(string source)
        {
            ...
            if (dosyaSonYazilmaTarihi < dosyaTarihi)
            {
                try
                {
                    EventLog.WriteEntry("AzureParser", "Dosya geldi (AA_Secim2015_SecimCevresiGumrukDahil.txt) " + DateTime.Now.ToShortTimeString() + "  /  " + +process.Id, EventLogEntryType.Information, 40);

                    FileInfo fileInfo = new FileInfo(source);
                    DateTime dosyaOlusturulmaTarihi = File.GetLastWriteTime(source);
                    gelenDosyaAdi = "AA_Secim2015_SecimCevresiGumrukDahil_" + dosyaOlusturulmaTarihi.Year + "_" + dosyaOlusturulmaTarihi.Month + "_" + dosyaOlusturulmaTarihi.Day + "_" + dosyaOlusturulmaTarihi.Hour + "_" + dosyaOlusturulmaTarihi.Minute + "_" + dosyaOlusturulmaTarihi.Second + "_" + dosyaOlusturulmaTarihi.Millisecond;
                    ...
                    EventLog.WriteEntry("AzureParser", "Dosya Okundu - Kopyalandı (AA_Secim2015_SecimCevresiGumrukDahil.txt) " + DateTime.Now.ToShortTimeString() + "  /  " + process.Id, EventLogEntryType.Information, 10);
        ...
        public static void ToCacheIlDagilimi()
        {
            if (JsonString == null)
            {
                bilgilendirme.EventlogaYaz("ToCacheIlDagilimGumrukDahil null", EventLogEntryType.Warning, 6);
            }
            else
            {
                try
                {
                    ...
                    cache.Put("GenelGumrukDahil", JsonConvert.SerializeObject(JsonString["AA"]).Replace("@", ""));
                    cache.Put("GenelUstVeri", JsonConvert.SerializeObject(JsonString["AA"]["TurkiyeGeneli"]).Replace("@", ""));
                    ...
                    cache.Put("TurkiyeHaritaGumrukDahil", JsonConvert.SerializeObject(turkiyeIlVerileriListesi));
                    bilgilendirme.EventlogaYaz("Cache e yazıldı. Gunruk Dahil XML" + DateTime.Now.ToShortTimeString() + process.Id.ToString(), EventLogEntryType.Information, 20);
                }
                catch (Exception ex)
                {
        ...
        public static string BosXmlOku(string source)
        {
            ...
            try
            {
                EventLog.WriteEntry("AzureParser", "Bos Xml Okundu (AA_Secim2015_IldagilimGumruk.txt) " + DateTime.Now.ToShortTimeString() + "  /  " + +process.Id);
                    ...
                    if (File.Exists(Onbellek.logPath + gelenDosyaAdi + ".txt"))
                    {
                        if (!File.Exists(Onbellek.logPath + gelenDosyaAdi + ".zip"))
                        {
                            try
                            {
                                ...
                                {
                                    zip.CreateEntryFromFile(Path.Combine(Onbellek.logPath, gelenDosyaAdi + ".txt"), "AA_Secim2015_SecimCevresiGumrukDahil.txt");
                                }
