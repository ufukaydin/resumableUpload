...
{
    /// <summary>
    /// Summary description for YurtDisi
    /// </summary>
    public class YurtDisi : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            context.Response.Cache.VaryByParams["*"] = true;
                ...
                string veri = cache["Gumruk"] as string;
                if (veri == null)
                {
                    CacheItemPolicy policy = new CacheItemPolicy();
                    policy.RemovedCallback = new CacheEntryRemovedCallback(Catch);
                    policy.AbsoluteExpiration = DateTime.Now.AddSeconds(60);
                    var TumIller = data.Get("Gumruk");
                    if (TumIller != null)
                        cache.Set("Gumruk", TumIller, policy);
