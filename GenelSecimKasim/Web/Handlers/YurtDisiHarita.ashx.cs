...
{
    /// <summary>
    /// Summary description for YurtDisiHarita
    /// </summary>
    public class YurtDisiHarita : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            string veri = cache["YurDisiHarita"] as string;
            if (veri == null)
            {
                CacheItemPolicy policy = new CacheItemPolicy();
                policy.RemovedCallback = new CacheEntryRemovedCallback(Catch);
                policy.AbsoluteExpiration = DateTime.Now.AddSeconds(60);
                var TumIller = data.Get("YurDisiHarita");
                if (TumIller != null)
                    cache.Set("YurDisiHarita", TumIller, policy);
